...
export interface SendTrackingEmailResult {
  ok: boolean;
  id?: string;
  error?: string;
}
...
export async function sendTrackingEmail(
  ...
  if (!apiKey) {
    console.error('[sendTrackingEmail] RESEND_API_KEY não configurada');
    return { ok: false, error: 'RESEND_API_KEY missing' };
  }

  const { nomeCliente, emailCliente, codigoPedido } = params;
  ...
  const subject = '📦 Atualização do seu pedido — acompanhe aqui';
  const trackHref = `https://eletrojundiai.shop/rastreio/${encodeURIComponent(codigoPedido)}`;
...
<body style="margin:0;padding:0;background:#fdf6f8;font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#222;">
  <div style="max-width:560px;margin:0 auto;padding:24px 16px;">
    <div style="background:#ffffff;border-radius:16px;box-shadow:0 4px 20px rgba(200,63,112,0.08);overflow:hidden;">
      <div style="background:linear-gradient(135deg,#c83f70 0%,#e85a8a 100%);padding:32px 24px 24px;text-align:center;color:#fff;">
        ...
        <h1 style="margin:0 0 6px;font-size:26px;font-weight:800;letter-spacing:-0.3px;">Acompanhe seu pedido</h1>
        <p style="margin:0;font-size:15px;opacity:0.95;">Olá ${escapeHtml(nomeCliente || 'cliente')} 💖</p>
      </div>
      ...
      <div style="padding:20px 24px 28px;">
        <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#444;text-align:center;">
          Seu acompanhamento de pedido já está disponível. Clique no botão acima para acompanhar atualizações em tempo real.
        </p>
        ...
        <p style="margin:18px 0 0;font-size:13px;line-height:1.6;color:#777;text-align:center;">
          Você receberá atualizações sempre que o status do pedido mudar.
        </p>
      </div>
  ...
  const text = [
    ...
    `Seu acompanhamento de pedido já está disponível. Clique abaixo para acompanhar atualizações em tempo real.`,
    `Código do pedido: ${codigoPedido}`,
    `Acompanhar pedido: ${trackHref}`,
  ...
  try {
    ...
    if (error) {
      console.error('[sendTrackingEmail] erro Resend', error);
      return { ok: false, error: String((error as any)?.message || error) };
    }

    console.log('[sendTrackingEmail][OK]', { id: data?.id, to: emailCliente, codigoPedido });
