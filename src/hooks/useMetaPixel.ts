...
export const useMetaPixel = () => {
  ...
  const trackViewContent = useCallback((d: any) => trackEvent('ViewContent', {
    content_name: d.content_name, content_type: d.content_type || 'product',
    content_ids: d.content_ids, value: d.value, currency: d.currency || 'BRL',
  }), [trackEvent]);
  ...
  const trackInitiateCheckout = useCallback((d: any) => trackEvent('InitiateCheckout', {
    content_name: d.content_name, content_ids: d.content_ids,
    value: d.value, currency: d.currency || 'BRL', num_items: d.num_items || 1,
  }), [trackEvent]);

  const trackLead = useCallback((d: any) => trackEvent('Lead', {
    content_name: d.content_name, content_category: d.content_category,
    value: d.value || 0, currency: d.currency || 'BRL',
  }), [trackEvent]);
  ...
  return { trackPageView, trackViewContent, trackAddToCart, trackInitiateCheckout, trackLead, trackPurchase };
};
